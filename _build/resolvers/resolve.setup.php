...
switch (@$options[xPDOTransport::PACKAGE_ACTION]) {
	...
	case xPDOTransport::ACTION_UPGRADE:
		...
		$packages = array('translit');
		
		if (!empty($options['update_snippets'])) {
			foreach($options['update_snippets'] as $upd_snip) $packages[] = $upd_snip;
		}

		foreach ($packages as $package_name) {
			$installed = $modx->getObject('transport.modTransportPackage', array('package_name' => $package_name));			
			if (!$installed) {
			...
			else $modx->log(modX::LOG_LEVEL_INFO, "<b>{$package_name}</b> has already been installed.");
		}
